...
def main():
    ...
    a = p.parse_args(); data = make_synthetic(n=a.n,seed=a.seed); print(f"{data['n_samples']:,} policies, claim rate={data['positive_rate']:.2%}")
    ...
    for n,s in cv.items(): print(f"  {n:25s} Tweedie Dev.={s['tweedie_deviance']['mean']:.2f} \u00b1{s['tweedie_deviance']['std']:.4f}")
    best = min(b["results"],key=lambda n: b["results"][n]["metrics"]["tweedie_deviance"])
